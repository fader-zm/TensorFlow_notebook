## 七 根据用户行为数据创建ALS模型并召回商品

### 7.0 用户行为数据拆分

- 方便练习可以对数据做拆分处理

  - pandas的数据分批读取  chunk (块)厚厚的一块 相当大的数量或部分
...
df = spark.read.csv("hdfs://hadoop000:9000/datasets/behavior_log.csv", header=True)
...
# 从hdfs加载数据为dataframe，并设置结构
behavior_log_df = spark.read.csv("hdfs://hadoop000:8020/datasets/behavior_log.csv", header=True, schema=schema)
...
# 写入数据时才开始计算
cate_count_df.write.csv("hdfs://hadoop000:9000/preprocessing_dataset/cate_count.csv", header=True)
brand_count_df.write.csv("hdfs://hadoop000:9000/preprocessing_dataset/brand_count.csv", header=True)
```
...
# 从hdfs加载CSV文件
cate_count_df = spark.read.csv("hdfs://hadoop000:9000/preprocessing_dataset/cate_count.csv", header=True, schema=schema)
...
- 通常如果USER-ITEM打分数据应该是通过以下方式进行处理转换为USER-ITEM-MATRIX
...
model.transform
# 将模型进行存储
model.save("hdfs://hadoop000:8020/models/userCateRatingALSModel.obj")
# 测试存储的模型
from pyspark.ml.recommendation import ALSModel
# 从hdfs加载之前存储的模型
als_model = ALSModel.load("hdfs://hadoop000:8020/models/userCateRatingALSModel.obj")
...
# 对每个分片的数据进行处理 #mapPartition Transformation
# foreachPartition Action操作
